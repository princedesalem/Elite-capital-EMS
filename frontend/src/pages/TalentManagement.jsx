  ...
  return (
    <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
      <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Users size={16} style={{ color: '#475569' }} /></div>
        <div style={{ flex: 1 }}>
        ...
        <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: '0.72rem', fontWeight: 700, background: '#f1f5f9', color: '#475569' }}>
          {meeting.statut === 'termine' ? 'Terminé' : 'Planifié'}
        </span>
        <button onClick={() => setExpanded(!expanded)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: '#64748b', fontSize: '0.7rem' }}>
          {expanded ? '▲' : '▼'}
        </button>
        {canEdit && <button onClick={() => onEdit(meeting)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: '#475569', fontSize: '0.7rem' }}>Modifier</button>}
        {canEdit && <button onClick={() => onDelete(meeting.id)} style={{ background: '#fef2f2', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: ACCENT, fontSize: '0.7rem' }}>Suppr.</button>}
      </div>
      {expanded && (
        <div style={{ borderTop: '1px solid #f1f5f9', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {meeting.agenda && (
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#334155', marginBottom: 4 }}>AGENDA</div>
              <div style={{ fontSize: '0.82rem', color: '#475569', whiteSpace: 'pre-line' }}>{meeting.agenda}</div>
            </div>
          )}
          {meeting.notes && (
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#334155', marginBottom: 4 }}>NOTES & DÉCISIONS</div>
              <div style={{ fontSize: '0.82rem', color: '#475569', whiteSpace: 'pre-line', background: '#f8fafc', padding: '8px 10px', borderRadius: 6 }}>{meeting.notes}</div>
            </div>
          )}
          {meeting.actions && (
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#334155', marginBottom: 4 }}>ACTIONS À SUIVRE</div>
...
function GoalCard({ goal, onDelete, onStatusChange, canEdit }) {
  ...
  return (
    <div style={{ background: 'white', border: `1px solid ${conf.color}30`, borderLeft: `4px solid ${conf.color}`, borderRadius: 10, padding: '12px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {canEdit && (
          <select value={goal.statut} onChange={e => onStatusChange(goal.id, e.target.value)} style={{ padding: '4px 8px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: '0.75rem', background: 'white', color: conf.color, fontWeight: 700 }}>
            {Object.entries(GOAL_STATUTS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
          </select>
        )}
        {canEdit && <button onClick={() => onDelete(goal.id)} style={{ background: '#fef2f2', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: ACCENT, fontSize: '0.7rem' }}>Suppr.</button>}
      </div>
    </div>
  )
}
...
export default function TalentManagement() {
  ...
  const tabs = [{ id: 'meetings', label: 'Réunions 1-on-1' }, { id: 'goals', label: 'Objectifs de carrière' }]
    ...
    <div style={{ paddingBottom: 32 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Target size={22} /> Talent Management
            </h1>
            <p style={{ margin: '4px 0 0', fontSize: '0.85rem', opacity: 0.8 }}>Réunions 1-on-1, objectifs de carrière et développement individuel</p>
          </div>
      ...
      {isAdmin && (
        <div style={{ marginBottom: 14 }}>
          <select style={{ padding: '7px 10px', border: '1px solid #e2e8f0', borderRadius: 7, fontSize: '0.82rem', background: 'white' }} value={filterEmp} onChange={e => setFilterEmp(e.target.value)}>
        ...
        <>
          {showMeetForm && (
            <div className="form-card" style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                <h3 style={{ margin: 0, color: DARK, fontWeight: 700 }}>{editMeeting ? 'Modifier la réunion' : 'Nouvelle réunion 1-on-1'}</h3>
              ...
              <form onSubmit={submitMeeting}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                  ...
                  <button type="button" className="btn" onClick={() => { setShowMeetForm(false); setEditMeeting(null) }} style={{ background: '#f1f5f9', color: '#475569' }}>Annuler</button>
                </div>
              </form>
            </div>
          )}
          {filteredMeetings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', background: 'white', borderRadius: 10, border: '1px solid #e2e8f0' }}>
              <Calendar size={36} style={{ opacity: 0.3, marginBottom: 12 }} /><p style={{ margin: 0 }}>Aucune réunion 1-on-1</p>
            </div>
          ) : (
      ...
      {activeTab === 'goals' && (
        <>
          ...
          {showGoalForm && (
            <div className="form-card" style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                <h3 style={{ margin: 0, color: DARK, fontWeight: 700 }}>Nouvel objectif de carrière</h3>
              ...
              <form onSubmit={submitGoal}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                  ...
                  <button type="button" className="btn" onClick={() => setShowGoalForm(false)} style={{ background: '#f1f5f9', color: '#475569' }}>Annuler</button>
                </div>
              </form>
            </div>
          )}
          {filteredGoals.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', background: 'white', borderRadius: 10, border: '1px solid #e2e8f0' }}>
              <Target size={36} style={{ opacity: 0.3, marginBottom: 12 }} /><p style={{ margin: 0 }}>Aucun objectif de carrière</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {filteredGoals.map(g => (
