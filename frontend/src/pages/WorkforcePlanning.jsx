...
function BarSimple({ value, max, color = ACCENT }) {
  return (
    <div style={{ background: 'var(--bg)', borderRadius: 4, height: 8, overflow: 'hidden', flex: 1 }}>
...
export default function WorkforcePlanning() {
  ...
  const tabs = [{ id: 'overview', label: "Vue d'ensemble" }, { id: 'postes', label: "Postes planifiés" }, { id: 'headcount', label: "Effectif actuel" }]
  ...
  return (
    <div style={{ paddingBottom: 32 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
              <TrendingUp size={22} /> {"Planification des effectifs"}
            </h1>
            <p style={{ margin: '4px 0 0', fontSize: '0.85rem', opacity: 0.8 }}>{"Planification et suivi des effectifs"}</p>
          </div>
          {isAdmin && (
        ...
        <div className="form-card" style={{ marginBottom: 20 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button type="submit" className="btn btn-primary">{editPos ? "Mettre à jour" : "Ajouter un poste"}</button>
      ...
      {activeTab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
            <h3 style={{ margin: '0 0 14px', color: DARK, fontSize: '0.9rem', fontWeight: 700 }}>{"Postes par trimestre"}</h3>
            {QUARTERS.map(q => (
          ...
          {/* Par priorité */}
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
            <h3 style={{ margin: '0 0 14px', color: DARK, fontSize: '0.9rem', fontWeight: 700 }}>{"Postes par priorité"}</h3>
        ...
        <div>
          {positions.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px', color: '#94a3b8', background: 'var(--card)', borderRadius: 10, border: '1px solid var(--border)' }}>
              <Briefcase size={36} style={{ opacity: 0.3, marginBottom: 12 }} /><p style={{ margin: 0 }}>{"Aucun poste"}</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {positions.map(pos => {
                ...
                return (
                  <div key={pos.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderLeft: `4px solid ${pConf.color}`, borderRadius: 10, padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      {isAdmin && pos.statut !== 'pourvu' && (
                        <button onClick={() => markFilled(pos.id)} style={{ padding: '4px 8px', background: 'var(--bg)', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '0.75rem', color: '#021630', fontWeight: 700 }}>{"Marquer comme pourvu"}</button>
                      )}
                      {isAdmin && <button onClick={() => startEdit(pos)} style={{ padding: '4px 8px', background: 'var(--bg)', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: '0.75rem', color: '#475569' }}>{"Modifier"}</button>}
      ...
      {activeTab === 'headcount' && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
          <h3 style={{ margin: '0 0 14px', color: DARK, fontSize: '0.9rem', fontWeight: 700 }}>{"Effectif par entité"}</h3>
