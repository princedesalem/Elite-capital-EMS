...
export default function CongesPage() {
  ...
  const renderActionButtons = (item, isRecu) => {
    ...
    const eyeBtn = <button key="eye" onClick={(e) => { e.stopPropagation(); setDetailCongeItem(item) }} className="btn-ghost-primary" style={{ ...rowBtn, display: 'inline-flex', alignItems: 'center' }} title="Voir détails"><Eye size={12} /></button>
    const pdfBtn = isValid ? <button key="pdf" onClick={(e) => { e.stopPropagation(); setDownloadingPdf(id); api.get(`/api/pdf/conges/${id}`, { responseType: 'blob' }).then(res => { const url = URL.createObjectURL(res.data); const a = document.createElement('a'); a.href = url; a.download = `conge_${id}.pdf`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }).finally(() => setDownloadingPdf(null)) }} className="btn-ghost-primary" style={{ ...rowBtn, display: 'inline-flex', alignItems: 'center', opacity: downloadingPdf === id ? 0.6 : 1 }} disabled={downloadingPdf === id} title="Télécharger PDF"><FileDown size={13} /></button> : null
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 980 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...th, width: '11%' }}>Demandeur</th>
                  <th style={{ ...th, width: '9%' }}>Statut</th>
                  <th style={{ ...th, width: '9%' }}>Date de création</th>
                  <th style={{ ...th, width: '9%' }}>Date de départ</th>
                  <th style={{ ...th, width: '9%' }}>Date retour</th>
                  <th style={{ ...th, width: '5%' }}>Durée</th>
                  <th style={{ ...th, width: '18%' }}>État</th>
              ...
              <tbody>
                {workflowPcaAg.map(item => (
                  ...
                  >
                    ...
                    <td style={td} onClick={(e) => e.stopPropagation()}>{renderEtatCell(item)}</td>
