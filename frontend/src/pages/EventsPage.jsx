  ...
  return (
    <div style={{ padding: '0 0 32px 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Brouillons', count: counts.brouillon, color: '#64748b', bg: '#f8fafc' },
          { label: 'En attente', count: counts.en_attente, color: '#94a3b8', bg: '#f8fafc' },
          { label: 'Approuvés', count: counts.approuve, color: '#021630', bg: '#f8fafc' },
        ].map(s => (
      ...
      {showForm && (
        <div className="form-card" style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ margin: 0, color: '#021630', fontWeight: 700 }}>{editingId ? 'Modifier' : 'Nouvel événement'}</h3>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              <div className="form-group">
                <label>Date début <span style={{ color: '#ce2b2b' }}>*</span></label>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input style={{ padding: '7px 12px', border: '1px solid #e2e8f0', borderRadius: 7, fontSize: '0.85rem', minWidth: 200 }} placeholder="Rechercher..." value={searchQ} onChange={e => setSearchQ(e.target.value)} />
        ...
        <select style={{ padding: '7px 10px', border: '1px solid #e2e8f0', borderRadius: 7, fontSize: '0.82rem', background: 'white' }} value={filterType} onChange={e => setFilterType(e.target.value)}>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
          {filtered.map(ev => {
            ...
            return (
              <div key={ev.id} style={{ background: 'white', border: `1px solid ${st.color}33`, borderTop: `4px solid ${st.color}`, borderRadius: 10, padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748b', background: '#f1f5f9', padding: '2px 7px', borderRadius: 20 }}>{ev.type}</span>
